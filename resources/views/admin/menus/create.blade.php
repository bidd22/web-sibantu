...
<div class="max-w-2xl mx-auto space-y-6 antialiased">
    ...
    <div class="bg-white rounded-3xl border border-slate-100 shadow-xl shadow-slate-200/40 overflow-hidden">
        ...
        <div class="px-6 py-5 bg-gradient-to-r from-amber-50/60 to-slate-50/60 border-b border-slate-200/60">
            <h2 class="text-xl font-black text-slate-900">Tambah Menu Navigasi</h2>
            <p class="text-xs text-slate-500 font-medium mt-0.5">Buat tautan navigasi dinamis baru untuk diakses oleh warga.</p>
        </div>

        <!-- Form Body -->
        <form method="POST" action="{{ route('admin.menus.store') }}" class="p-6 space-y-5">
            @csrf
